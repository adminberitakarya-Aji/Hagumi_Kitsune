  ...
  switch (action) {
    ...
    case "inbox": {
      ...
      const requests = (data as RequestRow[]).map((r) => ({
        id: r.id,
        status: r.status,
        direction: r.from_id === userId ? "outgoing" : "incoming",
        // outgoing -> partner = gen penerima (to_gen) bila sudah accept
        partner: r.from_id === userId ? r.to_gen : r.from_gen,
        seed: r.seed,
    ...
    case "send": {
      ...
      if (friend.owner === userId) return errorJson("Itu kodemu sendiri");
      ...
      await supabase.from("pets_gen").upsert({ anon_id: userId, gen: myGen, updated_at: new Date().toISOString() });
      const inserted = await supabase
        .from("breeding_requests")
        .insert({ from_id: userId, to_id: friend.owner, from_gen: myGen, status: "pending" })
        .select("id")
        .single();
    ...
    case "claim": {
      ...
      const isFrom = row.from_id === userId;
